...
namespace WebApp
{
    public class Startup
    {
        ...
        {
            services.AddDbContext<MainContex>(options =>
            {
                options.UseSqlServer(Configuration.GetConnectionString("DefaultString"));
            });
            services.AddIdentity<Person, IdentityRole>()
                .AddEntityFrameworkStores<MainContex>().AddDefaultTokenProviders();
            //services.AddIdentity<Person, IdentityRole>().AddEntityFrameworkStores<MainContex>();
